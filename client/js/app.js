...
function setupTabs() {
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      tabs.forEach((t) => {
        t.style.borderBottom = t.dataset.tab === tabId ? "2px solid var(--accent)" : "none";
      });
...
function setupAuthUI() {
  ...
  // Обновить UI при изменении авторизации
  onAuthChange((user) => {
    if (user) {
...
function setupChat() {
  ...
  loadChatHistory().then((messages) => {
    ...
    if (messages.length === 0) {
      chatMessages.innerHTML = '<div class="chat-empty">Нет сообщений</div>';
    } else {
  ...
  onConnectionChange((connected) => {
    if (connected) {
      ...
      chatStatusText.textContent = "Отключено";
    }
  });

  // Обновить UI чата при изменении авторизации
  onAuthChange((user) => {
    if (user) {
  ...
  chatLoginBtn.addEventListener("click", login);
}

// Отрисовать сообщение чата
function renderChatMessage(message) {
  ...
  avatar.width = 32;
  avatar.height = 32;
  drawHead(avatar, message.username);
  ...
  username.addEventListener("click", () => openSkinPreviewDialog(message.username));
